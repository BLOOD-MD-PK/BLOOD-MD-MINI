...
const BOT_NAME = config.BOT_NAME || 'BLOOD-MD-MINI';
const OWNER_NAME = config.OWNER_NAME || 'ʙʟᴏᴏᴅ-ᴄᴏ';
const OWNER_NUMBER = config.OWNER_NUMBER || ['923188871441'];

// ========== CHANNEL SETTINGS ==========
const CHANNEL_JID = config.CHANNEL_JID || '120363410830712574@newsletter';
...
// ========== GROUP ADMINS (BLOOD-MD-MINI Style) ==========
...
function arslanLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [BLOOD-MD-MINI] ${new Date().toISOString()}: ${message}`);
}
...
// ========== EXTRACT MESSAGE BODY (BLOOD-MD-MINI Style) ==========
...
// ========== EXTRACT BUTTON ID (Blood-MD-Mini Style) ==========
...
*© Powered by ${ʙʟᴏᴏᴅ-ᴄᴏ}*`;
        ...
        if (!existingSession) {
            try {
                await conn.sendMessage(userJid, {
                    image: { url: config.IMAGE_PATH || 'https://files.catbox.moe/5c2xf1.png' },
    ...
    if (connection === 'close') {
        ...
        // ========== MESSAGE HANDLER (blood-MD-mini Style) ==========
        ...
        arslanLog(`BLOOD-MD-MINI Pair error: ${err.message}`, 'error');
